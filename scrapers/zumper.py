...
Zumper often shows a whole apartment BUILDING with a price/bed range
across floor plans (e.g. "Studio-2 beds", "$3,960-$6,690") rather than
one specific unit. is_range detects this (multiple price figures, or a
dash inside the beds text) so filters.py can exclude building-level
listings entirely per the user's "individual properties only" request --
this is separate from parse_price() taking the low end for the
price/beds thresholds on listings that *do* pass.
...
# Specifically "$X-$Y" adjacent, not just "two $ figures somewhere in the
# card" -- a single unit with a struck-through "price drop" (original +
# discounted price) also has two $ figures but isn't a floor-plan range.
...
def scrape(site_config: dict) -> list[dict]:
    ...
    for a in soup.select("a[href*='/apartment-buildings/'], a[href*='/address/']"):
        ...
        is_range = bool(PRICE_RANGE_RE.search(text)) or bool(beds_text and re.search(r"[–-]", beds_text))
